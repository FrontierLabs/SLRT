    ...
    def load_keypoints(self):
        if 'keypoint' in self.input_streams:
            ...
            for name, all_keypoints in name2all_keypoints.items():
                ...
                # 获取实际的关键点数据
                if 'keypoints' in all_keypoints:
                    keypoints_data = all_keypoints['keypoints']  # 从字典中获取实际的关键点数组
                else:
                    keypoints_data = all_keypoints
                ...
                for k in sorted(self.dataset_cfg['use_keypoints']):
                    ...
                    selected_index = Part2index[k][0][1]
                    self.name2keypoints[name].append(keypoints_data[:, selected_index])  # T, N, 3
