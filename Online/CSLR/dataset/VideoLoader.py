...
def read_img(path, dataset_name, csl_cut=False, csl_resize=-1):
    ...
    if dataset_name.lower() in ['csl', 'csl_iso', 'cslr', 'ce_csl', 'ce_csl_iso']:  # cslr won'r enter here
        if csl_cut:
...
def read_jpg(zip_file, dataset_name, decoded_frames, seq_len, img_dir):
    ...
    for f in decoded_frames:
        ...
        if dataset_name.lower() in ['csl', 'csl_iso', 'csl_kp']:
            img_path = '{}/{}/{:06d}.jpg'.format(zip_file, img_dir, f)
            # img_path = '{}@sentence_frames-512x512/{}/{:06d}.jpg'.format(zip_file, img_dir, f)
        elif 'ce_csl' in dataset_name:
            img_path = '{}/{}/{:06d}.png'.format(zip_file, img_dir, f + 1)
...
def load_video(zip_file, name, vlen, raw_vlen, num_frames, dataset_name, is_train,
    ...
    if dataset_name in ['phoenix_iso', 'phoenix2014_iso', 'phoenix_comb_iso', 'csl_iso', 'ce_csl_iso']:
    ...
    if 'WLASL' in dataset_name or 'SLR500' in dataset_name:
    ...
    elif dataset_name in ['ce_csl_kp']:
        cut_num = int(name.split('/')[3].replace('cut_', ''))
        selected_index = selected_index + cut_num * 64
        # print(f'name: {name}; selected_index: {selected_index}')
        video_arrays = read_jpg(zip_file, dataset_name, selected_index, vlen, ori_vfile)
            ...
            dataset_name in ['phoenix_iso', 'phoenix2014_iso', 'phoenix_comb_iso', 'phoenix', 'phoenix2014',
                             'phoenixcomb',
                             'csl', 'csl_iso', 'csl_kp', 'ce_csl', 'ce_csl_iso']:
...
def load_batch_video(zip_file, names, vlens, raw_vlens, dataset_name, is_train,
    ...
    for n_frames, f in zip(num_output_frames, fps):
        ...
        for name, vlen, raw_vlen, ori_vfile in zip(names, vlens, raw_vlens, ori_video_files):
            video, selected_index, pad = load_video(zip_file, name, vlen, raw_vlen, n_frames, dataset_name, is_train,
                                                    index_setting, temp_scale, ori_vfile)
            # video = torch.tensor(video).to(torch.uint8)
            video = torch.tensor(video).float()  # T,H,W,C
            if 'NMFs-CSL' in dataset_name:
                video = torchvision.transforms.functional.resize(video.permute(0, 3, 1, 2), [256, 256]).permute(0, 2, 3,
                                                                                                                1)
            video /= 255
            ...
            if name2keypoint != None:
                ...
                elif dataset_name in ['ce_csl', 'ce_csl_iso']:
                    cut_keypoint = {}
