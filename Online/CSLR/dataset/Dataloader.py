...
def collate_fn_(batch, data_cfg, is_train, vocab, name2keypoint, word_emb_tab, vfile2raw_vlens,
    ...
    outputs = {'names': [sample['name'] for sample in batch],
               'word_embs': None if word_emb_tab is None else torch.stack(
                   [torch.from_numpy(word_emb_tab[sample['label']]) for sample in batch], dim=0),
               'labels': [vocab.index(sample['label']) for sample in batch] if data_cfg['dataset_name'] not in [
                   'phoenix', 'phoenix2014', 'phoenixcomb', 'csl', 'csl_kp', 'ce_csl', 'ce_csl_kp'] else [0 for sample in batch],
               ...
               'vlens': [sample['seq_len'] for sample in batch] if data_cfg['dataset_name'] not in ['phoenix',
                                                                                                    'phoenix2014',
                                                                                                    'phoenixcomb',
                                                                                                    'csl',
                                                                                                    'csl_kp',
               ...
               'ori_video_files': [sample['video_file'] for sample in batch] if data_cfg['dataset_name'] not in [
                   'phoenix', 'phoenix2014', 'phoenixcomb', 'csl', 'csl_kp'] else [sample['name'] for sample in batch],
               'gls_ref': [sample['gloss'] for sample in batch] if data_cfg['dataset_name'] in ['phoenix',
                                                                                                'phoenix2014',
                                                                                                'phoenixcomb',
                                                                                                'csl',
                                                                                                'csl_kp',
    ...
    if task == 'ISLR':
        ...
        if 'base_start' in batch[0]:
            ...
            for sample in batch:
                start, end, vlen, base_start, base_end = sample['start'], sample['end'], sample['seq_len'], sample[
                    'base_start'], sample['base_end']
                if sample['aug'] == 0:
    ...
    if task == 'G2G':
        ...
        if g2g_input_type == 'gloss':
            ...
            outputs['translation_inputs'].update(
                g2g_tokenizer(input_gls_seq, label_gls_seq, blank_as_mask, need_input=True, need_label=False))
...
def build_dataloader(cfg, split, task='ISLR', g2g_tokenizer=None, is_train=True, val_distributed=False):
    ...
    collate_func = partial(collate_fn_, data_cfg=cfg['data'], is_train=is_train,
                           ...
                           task=task,
                           g2g_input_type=cfg['model']['TranslationNetwork']['input_type'] if 'TranslationNetwork' in
                                                                                              cfg['model'] else 'gloss',
                           g2g_tokenizer=g2g_tokenizer, g2g_input=dataset.g2g_input,
                           vfile2framelabel=dataset.vfile2framelabel)
    if is_train:
