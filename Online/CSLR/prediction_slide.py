...
def sliding_windows(video, keypoint, win_size=16, stride=1, save_fea=False):
    ...
    # make slided inputs
    video_s = torch.zeros(num_clips, win_size, video.shape[1], video.shape[2], video.shape[3]).to(
        video.device)  # N,W,C,H,W
...
def evaluation_slide(model, cslr_dataloader, cfg,
    ...
    if is_main_process() and os.environ.get('enable_pbar', '1') == '1':
        ...
        pbar = None
    if epoch != None:
        logger.info('------------------Evaluation epoch={} {} examples #={}---------------------'.format(epoch,
                                                                                                         cslr_dataloader.dataset.split,
                                                                                                         len(cslr_dataloader.dataset)))
    elif global_step != None:
        logger.info('------------------Evaluation global step={} {} examples #={}------------------'.format(global_step,
                                                                                                            cslr_dataloader.dataset.split,
                                                                                                            len(cslr_dataloader.dataset)))
    ...
    decode_method_lst = ['window_greedy_3', 'window_greedy_5', 'window_greedy_7', 'window_greedy_9', 'window_greedy_11',
                         'window_greedy_13', 'naive_greedy']
    ...
    with torch.no_grad():
        ...
        for thr in thr_lst:
            ...
            for step, batch in enumerate(cslr_dataloader):
                ...
                video_s, keypoint_s = sliding_windows(batch['sgn_videos'][0], batch['sgn_keypoints'][0],
                                                      win_size=win_size, stride=stride, save_fea=save_fea)
                video_s_lst, keypoint_s_lst = video_s.split(split_size, dim=0), keypoint_s.split(split_size,
                                                                                                 dim=0)  # S,W,C,H,W
                ...
                for v_s, k_s in zip(video_s_lst, keypoint_s_lst):
                    v_s, k_s, batch['labels'] = v_s.to(cfg['device']), k_s.to(cfg['device']), batch['labels'].to(
                        cfg['device'])
                    if len(cfg['data']['input_streams']) == 2:
                        ...
                        sgn_videos = [v_s]
                        sgn_videos.append(
                            sgn_videos[-1][:, win_size // 4:win_size // 4 + win_size // 2, ...].contiguous())
                        sgn_keypoints = [k_s]
                        sgn_keypoints.append(
                            sgn_keypoints[-1][:, win_size // 4:win_size // 4 + win_size // 2, ...].contiguous())

                    forward_output = model(is_train=False, labels=batch['labels'], sgn_videos=sgn_videos,
                                           sgn_keypoints=sgn_keypoints, epoch=epoch)
                ...
                if save_fea or use_bag_fc or model_ex:
                    ...
                    feas_rgb = feas_rgb.view(feas_rgb.size(0), feas_rgb.size(1), feas_rgb.size(2)).mean(
                        dim=-1)  # real_T,C
                    ...
                    feas_rgb_blk5 = F.avg_pool3d(feas_rgb_blk5, (2, feas_rgb_blk5.size(3), feas_rgb_blk5.size(4)),
                                                 stride=1)
                    feas_rgb_blk5 = feas_rgb_blk5.view(feas_rgb_blk5.size(0), feas_rgb_blk5.size(1),
                                                       feas_rgb_blk5.size(2)).mean(dim=-1)  # real_T,C
                    ...
                    feas_kp_blk5 = feas_kp_blk5.view(feas_kp_blk5.size(0), feas_kp_blk5.size(1),
                                                     feas_kp_blk5.size(2)).mean(dim=-1)  # real_T,C
                ...
                if 'window_greedy_7' in decode_method_lst:
                    ...
                    for decode_win_size in [3, 5, 7, 9, 11, 13]:
                        ...
                        if use_bag_fc or model_ex:
                            rgb_feas = pad_tensor(feas['rgb_blk5'][name], decode_win_size // 2,
                                                  decode_win_size // 2).to(model.device)
                            kp_feas = pad_tensor(feas['keypoint_blk5'][name], decode_win_size // 2,
                                                 decode_win_size // 2).to(model.device)
                        ...
                        for st in range(index.shape[0] - decode_win_size + 1):
                            ...
                            if use_bag_fc or model_ex:
                                if use_bag_fc:
                                    cur_rgb_feas = rgb_feas[st:st + decode_win_size].contiguous().mean(dim=0,
                                                                                                       keepdim=True)  # 1,C
                                    cur_pose_feas = kp_feas[st:st + decode_win_size].contiguous().mean(dim=0,
                                                                                                       keepdim=True)  # 1,C
                                    ...
                                    logits = model_ex(is_train=False, labels=batch['labels'], sgn_videos=sgn_videos,
                                                      sgn_keypoints=sgn_keypoints, epoch=epoch,
                                                      denoise_inputs={'features': cur_fuse_feas, 'labels': None})[
                                        'logits']
    ...
    logger.info('-------------------------Evaluation Finished-------------------------'.format(global_step,
                                                                                               len(cslr_dataloader.dataset)))
...
def eval_g2g(model, val_dataloader, cfg,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(val_dataloader):
            ...
            names = batch['names']
            batch = move_to_device(batch, cfg['device'])
            forward_output = model(is_train=False, labels=batch['labels'], sgn_videos=batch['sgn_videos'],
                                   sgn_keypoints=batch['sgn_keypoints'], epoch=epoch,
...
if __name__ == "__main__":
    ...
    for split in [args.split]:
        ...
        dataloader, sampler = build_dataloader(cfg, split, task=cfg['task'], g2g_tokenizer=g2g_tokenizer,
                                               is_train=False, val_distributed=False)
