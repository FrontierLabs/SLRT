...
if __name__ == "__main__":
    ...
    parser.add_argument('--close_wandb', action='store_true', help='Set to True to disable wandb')
    ...
    train_dataloader, train_sampler = build_dataloader(cfg, 'train', task=cfg['task'], g2g_tokenizer=g2g_tokenizer,
                                                       is_train=True)
    dev_dataloader, dev_sampler = build_dataloader(cfg, 'dev', task=cfg['task'], g2g_tokenizer=g2g_tokenizer,
                                                   is_train=False, val_distributed=(cfg['task'] in ['ISLR']))
    ...
    # OPTIMIZATION
    optimizer = build_optimizer(config=cfg['training']['optimization'],
                                model=model.module if not use_amp else model)  # assign different learning rates for different modules
    ...
    if cfg['training'].get('from_ckpt', False):
        ...
        if os.path.exists(os.path.join(model_dir, 'ckpts')):
            ckpt_lst = sorted([f for f in os.listdir(os.path.join(model_dir, 'ckpts')) if
                               'best' not in f and 'first_update' not in f])
            if len(ckpt_lst) > 0:
                ...
                start_epoch = state_dict['epoch'] + 1 if state_dict['epoch'] is not None else int(
                    latest_ckpt.split('_')[-1][:-5]) + 1
                ...
                train_dataloader, train_sampler = build_dataloader(cfg, 'train', task=cfg['task'],
                                                                   g2g_tokenizer=g2g_tokenizer, is_train=True)
                dev_dataloader, dev_sampler = build_dataloader(cfg, 'dev', task=cfg['task'],
                                                               g2g_tokenizer=g2g_tokenizer, is_train=False,
                                                               val_distributed=(cfg['task'] in ['ISLR']))
    ...
    contras_setting = cfg['model']['RecognitionNetwork']['visual_head']['contras_setting'] if cfg[
                                                                                                  'task'] == 'ISLR' else None
    ...
    for epoch_no in range(start_epoch, total_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if use_amp == 'torch':
                with autocast():
                    output = model(is_train=True, labels=batch['labels'], sgn_videos=batch['sgn_videos'],
                                   sgn_keypoints=batch['sgn_keypoints'], epoch=epoch_no)
            else:
                output = model(is_train=True, labels=batch['labels'], sgn_videos=batch['sgn_videos'],
                               sgn_keypoints=batch['sgn_keypoints'], epoch=epoch_no,
                               fbank_rgb=fbank_rgb, fbank_pose=fbank_pose, start_idx=batch['start_idx'],
                               aug=batch['aug'], bag_labels=batch['bag_labels'],
                               iou_labels=batch['iou_labels'], temp_idx=batch['temp_idx'],
            ...
            if contras_setting is not None and 'ema' in contras_setting:
                ...
                if cfg['data']['input_streams'] == ['rgb']:
                    ...
                    cur_model_lst.append(
                        model.module.recognition_network.visual_head_keypoint.word_fused_gloss_output_layer)
                elif len(cfg['data']['input_streams']) == 2:
                    language_apply_to = cfg['model']['RecognitionNetwork'].get('language_apply_to',
                                                                               'rgb_keypoint_joint_traj')
                    if 'rgb' in language_apply_to:
                        ma_model_lst.append(model.module.recognition_network.visual_head.gloss_output_layer)
                        cur_model_lst.append(model.module.recognition_network.visual_head.word_fused_gloss_output_layer)
                        if 'xmodal' in contras_setting:
                            extra_model_lst.append(
                                model.module.recognition_network.visual_head.xmodal_fused_gloss_output_layer)
                    if 'keypoint' in language_apply_to:
                        ...
                        if 'xmodal' in contras_setting:
                            extra_model_lst.append(
                                model.module.recognition_network.visual_head_keypoint.xmodal_fused_gloss_output_layer)
                    if 'joint' in language_apply_to:
                        ma_model_lst.append(model.module.recognition_network.visual_head_fuse.gloss_output_layer)
                        cur_model_lst.append(
                            model.module.recognition_network.visual_head_fuse.word_fused_gloss_output_layer)
                        if 'xmodal' in contras_setting:
                    ...
                    if 'traj' in language_apply_to:
                        ...
                        cur_model_lst.append(
                            model.module.recognition_network.visual_head_traj.word_fused_gloss_output_layer)
                    ...
                    for k, v in model.module.recognition_network.head_dict.items():
                        if v is not None:
                            if ('rgb' in language_apply_to and k in ['rgb-h', 'rgb-l']) \
                                    ...
                                    or ('joint' in language_apply_to and k in ['fuse', 'fuse-h', 'fuse-l', 'fuse-x-rgb',
                                                                               'fuse-x-kp']):
    ...
    if cfg['task'] == 'G2G' and is_main_process():
        ...
        test_dataloader, test_sampler = build_dataloader(cfg, 'test', task=cfg['task'], g2g_tokenizer=g2g_tokenizer,
                                                         is_train=False, val_distributed=(cfg['task'] in ['ISLR']))
